...
const CairnMark = ({ size = 1 }) => (
  <svg width={48 * size} height={52 * size} viewBox="0 0 48 52" fill="none">
    {[
      ...
      <rect key={i} x={(48 - bar.w) / 2} y={2 + i * 13} width={bar.w} height={7} rx={3.5} fill={bar.color} opacity={0.9} />
    ))}
  </svg>
);
    ...
    <div style={S.page}>
      ...
      <nav style={{
        ...
        <div style={{ display: "flex", gap: 32, alignItems: "center" }}>
          {["Produkt", "For hvem", "Om oss"].map(item => (
            <span key={item} style={{ fontSize: 13, color: "#7a8599", cursor: "pointer", fontWeight: 500, letterSpacing: "0.02em", transition: "color 0.2s" }}
              onMouseEnter={e => e.target.style.color = "#e8ecf4"}
          ...
          <span style={{
            ...
            onMouseLeave={e => { e.target.style.background = "#e8ecf4"; e.target.style.color = "#0a0f1a"; }}
          >Kom i gang</span>
        </div>
      </nav>
        ...
        <div style={{ maxWidth: 900, margin: "0 auto" }}>
          ...
          <FadeIn delay={0.2}>
            <div style={{ display: "flex", gap: 1, marginTop: 80, justifyContent: "center" }}>
              {[
                { label: "Strategi", sub: "Vedtatt og forankret", opacity: 1 },
                { label: "???", sub: "Det alle mangler", opacity: 0.3, highlight: true },
                { label: "Eksekvering", sub: "Prosjekter og team", opacity: 1 },
              ].map((item, i) => (
      ...
      {/* ─── Four dimensions ─── */}
      <section style={{ padding: "100px 48px 140px", position: "relative" }}>
                ...
                <div key={item.label} style={{ display: "flex", alignItems: "center" }}>
                  ...
                  {i < 2 && (
                    <div style={{ display: "flex", alignItems: "center", width: 60 }}>
                      ...
                      <div style={{ color: "#3a4558", fontSize: 16, margin: "0 4px" }}>→</div>
                      <div style={{ flex: 1, height: 1, background: "rgba(255,255,255,0.1)" }} />
                    </div>
                  )}
                </div>
      ...
      <section style={{ padding: "100px 48px 140px" }}>
        <div style={{ maxWidth: 1000, margin: "0 auto" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 48 }}>
            {[
              {
                title: "Flytt en stein. Se hvilke stier som kollapser.",
                desc: "Dra en aktivitet til en annen horisont. Cairn viser umiddelbart hvilke avhengigheter som brytes og hvilke effekter som forsvinner. Konsekvensene er synlige før du forplikter deg.",
                tag: "Avhengigheter",
              },
              {
                title: "To strategier. Ett budsjett. Sammenlikn.",
                desc: "Dupliser et scenario, endre prioriteringene, og se forskjellene side om side. Hvilke effekter mister vi? Hvilke dimensjoner blir dekket? Velg med åpne øyne.",
                tag: "Scenarier",
              },
              {
                title: "IT har 11 samtidige aktiviteter. Ser det?",
                desc: "Cairn viser endringsbelastning per eier. Den vanligste årsaken til at transformasjoner feiler er ikke feil strategi — det er overbelastning.",
                tag: "Kapasitet",
              },
              {
                title: "Modenhet nå vs. modenhet etter.",
                desc: "Toggle simulering for å se hva kapabilitetskartet ser ut etter gjennomføring. Hver aktivitet har en forventet effekt. Styringsgruppen ser konkret hva investeringen leverer.",
                tag: "Simulering",
              },
            ].map((f, i) => (
            ...
            {[
              ...
              <FadeIn key={p.role} delay={i * 0.1}>
                <div style={{
                  ...
                  <p style={{ fontSize: 12, color: "#818cf8", fontWeight: 600, marginBottom: 14 }}>Åpner {p.opens}</p>
        ...
        <div style={{ maxWidth: 700, margin: "0 auto", textAlign: "center" }}>
          <FadeIn>
            <p style={{ ...S.serif, fontSize: 32, fontStyle: "italic", color: "#c8d0de", lineHeight: 1.45, marginBottom: 24 }}>
              "A cairn doesn't need a legend.<br />Neither should your strategy."
            </p>
          </FadeIn>
        </div>
      </section>
        ...
        <div style={{ maxWidth: 600, margin: "0 auto", textAlign: "center", position: "relative", zIndex: 2 }}>
          ...
          <FadeIn delay={0.3}>
            <span style={{
              ...
              onMouseEnter={e => { e.target.style.background = "#818cf8"; e.target.style.color = "#fff"; e.target.style.transform = "translateY(-2px)"; }}
              onMouseLeave={e => { e.target.style.background = "#e8ecf4"; e.target.style.color = "#0a0f1a"; e.target.style.transform = "translateY(0)"; }}
            >Prøv Cairn gratis</span>
          </FadeIn>
        </div>
      </section>
      ...
      <footer style={{ padding: "48px 48px 40px", borderTop: "1px solid rgba(255,255,255,0.05)" }}>
        <div style={{ maxWidth: 1000, margin: "0 auto", display: "flex", justifyContent: "space-between", alignItems: "flex-end" }}>
          <div>
            ...
            <p style={{ fontSize: 11, color: "#3a4558", margin: 0 }}>Navigate the fog. © 2026</p>
          </div>
          <div style={{ display: "flex", gap: 24 }}>
            {["Personvern", "Vilkår", "Kontakt"].map(item => (
              <span key={item} style={{ fontSize: 11, color: "#3a4558", cursor: "pointer", transition: "color 0.2s" }}
                onMouseEnter={e => e.target.style.color = "#7a8599"}
                onMouseLeave={e => e.target.style.color = "#3a4558"}
              >{item}</span>
            ))}
          </div>
        </div>
