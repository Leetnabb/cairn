...
function buildCsv(header: string[], rows: string[][]): string {
  return BOM + [header, ...rows].map(r => r.map(escapeCsv).join(';')).join('\n');
}
...
export function exportCapabilitiesCsv(capabilities: Capability[]) {
  ...
  downloadCsv(buildCsv(header, rows), i18n.t('export.csvCapFilename'));
}
...
export function exportInitiativesCsv(initiatives: Initiative[], filters?: ExportFilters) {
  ...
  downloadCsv(buildCsv(header, rows), i18n.t('export.csvInitFilename'));
}
...
export function exportEffectsCsv(effects: Effect[]) {
  ...
  downloadCsv(buildCsv(header, rows), i18n.t('export.csvEffFilename'));
}
